...
var GrindTrickRandomizer = /*#__PURE__*/function () {
  ...
  _createClass(GrindTrickRandomizer, [{
    ...
    value: function getTricktionaryEntries(slots, result) {
      ...
      if (approach) {
        var name = approach.winner.name;
        var switchTxt = name.includes("Switch") ? "<b>Switch</b>: ".concat(CONFIG.GLOSSARY["Switch"], " <br/>") : "";
        switchTxt = switchTxt === "" && name.includes("Natural") ? "<b>Natural</b>: ".concat(CONFIG.GLOSSARY["Natural"], " <br/>") : switchTxt;
        var fakieTxt = name.includes("Fakie") ? "<b>Fakie</b>: ".concat(CONFIG.GLOSSARY["Fakie"], " <br/>") : "";
        fakieTxt = fakieTxt === "" && name.includes("Forwards") ? "<b>Forwards</b>: ".concat(CONFIG.GLOSSARY["Forwards"], " <br/>") : fakieTxt;
        html += " ".concat(switchTxt).concat(fakieTxt, "  ");
      }

      if (spinTo) {
        var _name = spinTo.winner.name;
        var inSpinTxt = _name.includes("Inspin") ? "<b>Inspin</b>: ".concat(CONFIG.GLOSSARY["Inspin"], " <br/>") : "";
        var outSpinTxt = _name.includes("Outspin") ? "<b>Outspin</b>: ".concat(CONFIG.GLOSSARY["Outspin"], " <br/>") : "";
        html += " ".concat(inSpinTxt).concat(outSpinTxt, "  ");
      }

      if (grind) {
        var _name2 = grind.winner.name;
        var grindData = CONFIG.GRINDS.filter(function (g) {
          return g.name === _name2;
        })[0];
        html += "<b>".concat(grindData.name, "</b>: ").concat(grindData.comment ? grindData.comment + "<br>" : "", "\n       ").concat(grindData.thumbnailUrl ? "<img height=250 width=250 src='" + grindData.thumbnailUrl + "'></img" : "", "     ");
      }

      if (grindVariation) {
        var _name3 = grindVariation.winner.name; // "combos: split them in two"

        var varData = CONFIG.VARIATIONS.filter(function (g) {
          return g.name === _name3;
        })[0];
        html += "<b>".concat(_name3, "</b>: ").concat(varData.comment, "  ");
      }
...
var SlotMachine = /*#__PURE__*/function () {
  ...
  _createClass(SlotMachine, [{
    ...
    value: function run() {
      ...
      this.onSpinStart(); // this.spinWinners = this.keepLockedSpinWinners();

      console.log(" --------------- run  ---------------"); //console.table('run: keepLockedSpinWinners', this.spinWinners);

      console.table("run: slots", this.slots);
      console.table("run: disabled slots", this.slots.filter(function (s) {
    ...
    value: function onCompleteSlot(resolve, slot, activeNodeIndex) {
      ...
      if (slot.name === "Grind") {
        ...
        this.slots[3].data = this.trickdata.getSpinToData(grindSlot.winner.winner); // spinOff

        this.slots[4].data = this.trickdata.getSpinOffData(grindSlot.winner.winner);
      }
      ...
      if (slot.name === "Approach") {
        ...
        this.slots[3].data = this.trickdata.getSpinToData(_grindSlot.winner.winner, approachSlot.winner.winner); // spinOff

        this.slots[4].data = this.trickdata.getSpinOffData(_grindSlot.winner.winner);
      }
    ...
    value: function filterLocked(entries) {
      ...
      if (spinToSlot.state === SLOT_STATES.locked) {
        var isSoulSpin = this.isSoulSpin(spinToSlot.winner.winner.name);
        entries = entries.filter(function (e) {
      ...
      if (spinOffSlot.state === SLOT_STATES.locked) {
        var _isSoulSpin = this.isSoulSpin(spinOffSlot.winner.winner.name);
...
var Tooltips = /*#__PURE__*/function () {
  ...
  _createClass(Tooltips, [{
    ...
    value: function hide() {
      ...
      this.helpTooltips.forEach(function (t) {// t.instance.hide();
        // t.instance.disable();
      });
      this.$mask.hide();
    }
...
GRINDS_FOR_SLOTS.forEach(function (g, index) {
  if (g.variations) {
    (function () {
      ...
      /*
      let keys = Object.keys(g.variations);
      keys.forEach((key) => {
        let obj = { name: key };
        arrayHash.push(obj);
      });
  ...
  _createClass(Tricknames, [{
    ...
    value: function renderGrinds() {
      ...
      grinds.forEach(function (grind) {
        ...
        rows.push("<tr class=\" \">\n    <td>".concat(grind.name, "</td>\n    <td>").concat(url).concat(comment, "</td>\n  </tr>"));
      });
    ...
    value: function renderGrindSynonyms() {
      var rows = [];
      var vars = CONFIG.GRIND_SYNONYMS;
      ...
      vars.forEach(function (variaton) {
        ...
        rows.push("<tr class=\" \">\n    <td>".concat(variaton.newName, "</td>\n    <td><a target=\"blank\" href=\"").concat(variaton.url, "\">").concat(url, "</a>").concat(comment, "</td>\n  </tr>"));
        rows = rows.sort();
      });
